...
function makeRequest(query = '') {
  return new Request(`https://wm.opsio.space/api/youtube/embed${query}`);
}
...
test('returns embeddable html for valid video id', async () => {
  ...
  assert.equal(html.includes('origin:"https://wm.opsio.space"'), true);
...
test('uses dedicated parentOrigin for iframe postMessage target', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk&origin=https://wm.opsio.space&parentOrigin=https://tauri.localhost'));
  const html = await response.text();
...
test('does not accept wildcard parentOrigin query parameter', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk&origin=https://wm.opsio.space&parentOrigin=*'));
  const html = await response.text();
