...
import { extendExistingTtl, getRedisCredentials, redisSet, redisGet } from './_seed-utils.mjs';
...
async function seedRedis(output) {
  ...
  const v2Result = await redisSet(redisUrl, redisToken, REDIS_KEY_V2, output, REDIS_TTL);
  console.error(`[gpsjam] Redis SET v2 result:`, v2Result);
  ...
  const v1Result = await redisSet(redisUrl, redisToken, REDIS_KEY_V1, v1Output, REDIS_TTL);
  console.error(`[gpsjam] Redis SET v1 result:`, v1Result);

  // Verify v2
  const parsed = await redisGet(redisUrl, redisToken, REDIS_KEY_V2);
  if (parsed) {
  ...
  await redisSet(redisUrl, redisToken, metaKey, meta, 604800).catch(() => console.error('[gpsjam] seed-meta write failed'));
