...
import { loadEnvFile, CHROME_UA, runSeed, writeExtraKeyWithMeta, sleep, getRedisCredentials, redisGet } from './_seed-utils.mjs';
...
async function fetchNotamClosures() {
  try {
    ...
    return await redisGet(url, token, 'aviation:notam:closures:v2');
  } catch {
    return null;
  }
}
