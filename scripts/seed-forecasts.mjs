...
import { loadEnvFile, runSeed, CHROME_UA, getRedisCredentials as sharedGetRedisCredentials, redisCommand as sharedRedisCommand, redisGet as sharedRedisGet, redisDel as sharedRedisDel, redisSet as sharedRedisSet, redisPipeline as sharedRedisPipeline } from './_seed-utils.mjs';
...
function getRedisCredentials() {
  return sharedGetRedisCredentials();
}
...
async function redisCommand(url, token, command) {
  return sharedRedisCommand(url, token, command);
}
...
async function redisDel(url, token, key) {
  return sharedRedisDel(url, token, key);
}
...
async function warmPingChokepoints() {
  ...
  try {
    const resp = await fetch(`${baseUrl}/api/supply-chain/v1/get-chokepoint-status`, {
      headers: { 'User-Agent': CHROME_UA, Origin: 'https://wm.opsio.space' },
...
async function readInputKeys() {
  ...
  const results = await sharedRedisPipeline(url, token, pipeline);
    ...
    try {
      const resp = await fetch(provider.apiUrl, {
        ...
        headers: {
          ...
          ...(provider.name === 'openrouter' ? { 'HTTP-Referer': 'https://wm.opsio.space', 'X-Title': 'World Monitor' } : {}),
        },
        body: JSON.stringify({
...
async function redisSet(url, token, key, data, ttlSeconds) {
  try {
    await sharedRedisSet(url, token, key, data, ttlSeconds);
