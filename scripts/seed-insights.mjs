...
import { loadEnvFile, CHROME_UA, getRedisCredentials, runSeed, redisGet } from './_seed-utils.mjs';
...
async function readDigestFromRedis() {
  ...
  return redisGet(url, token, DIGEST_KEY);
}

async function readExistingInsights() {
  const { url, token } = getRedisCredentials();
  return redisGet(url, token, CANONICAL_KEY);
}
