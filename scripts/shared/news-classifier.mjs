// Keyword-based threat classifier for the self-hosted digest seeder.
// Ported from server/worldmonitor/news/v1/_classifier.ts (classifyByKeyword)
// so seed-00-news-digest.mjs's items carry a `.threat` field — without it,
// downstream consumers (seed-insights.mjs, ThreatTimelinePanel) see every
// item as untagged and fall back to `llm=0 keywordFallback=0` structurally,
// not because classification failed but because it never ran at all.
// Keep this in sync with the .ts original if its keyword maps change.
