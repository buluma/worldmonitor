...
export class DataLoaderManager implements AppModule {
  ...
  async loadAllData(forceAll = false): Promise<void> {
    ...
    if (SITE_VARIANT !== 'happy' && (this.ctx.mapLayers.iranAttacks || shouldLoadAny(['cii', 'strategic-risk', 'strategic-posture']))) tasks.push({ name: 'iranAttacks', task: runGuarded('iranAttacks', () => this.loadIranEvents()) });
