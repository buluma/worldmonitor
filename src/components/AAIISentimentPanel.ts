...
function renderBar(pct: number, color: string, label: string, value: string): string {
  return `<div style="margin:4px 0">
    <div style="display:flex;justify-content:space-between;font-size:11px;color:var(--text-dim);margin-bottom:2px">
      <span>${escapeHtml(label)}</span><span style="color:${color};font-weight:600">${escapeHtml(value)}</span>
    </div>
...
function renderSparkChart(weeks: WeekData[]): string {
  ...
  const points = data.map((w, i) => `${(PAD + i * stepX).toFixed(1)},${(midY - w.spread * scaleY).toFixed(1)}`);
  const bars = data.map((w, i) => {
    const x = PAD + i * stepX - 1;
    const barH = Math.abs(w.spread) * scaleY;
    const y = w.spread >= 0 ? midY - barH : midY;
    return `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="2" height="${barH.toFixed(1)}" fill="${w.spread >= 0 ? 'rgba(39,174,96,0.25)' : 'rgba(231,76,60,0.25)'}" rx="0.5"/>`;
  }).join('');
  ...
  return `<div style="margin:8px 0">
    ...
    <svg viewBox="0 0 ${W} ${H}" width="100%" height="${H}" style="display:block">
      ${bars}
      <line x1="${PAD}" y1="${midY}" x2="${W - PAD}" y2="${midY}" stroke="rgba(255,255,255,0.15)" stroke-width="0.5" stroke-dasharray="3,3"/>
      <line x1="${PAD}" y1="${contrarian.toFixed(1)}" x2="${W - PAD}" y2="${contrarian.toFixed(1)}" stroke="rgba(231,76,60,0.3)" stroke-width="0.5" stroke-dasharray="2,4"/>
      <text x="${W - PAD}" y="${(contrarian - 2).toFixed(1)}" text-anchor="end" font-size="7" fill="rgba(231,76,60,0.5)" font-family="system-ui,sans-serif">-20 buy signal</text>
      <polyline points="${points.join(' ')}" fill="none" stroke="rgba(255,255,255,0.6)" stroke-width="1.2" stroke-linejoin="round"/>
    </svg>
  </div>`;
}
...
export class AAIISentimentPanel extends Panel {
  ...
  constructor() {
    super({ id: 'aaii-sentiment', title: t('panels.aaiiSentiment'), showCount: false, infoTooltip: t('components.aaiiSentiment.infoTooltip') });
  }
  ...
  public async fetchData(): Promise<boolean> {
    ...
    try {
      const resp = await fetch(toApiUrl('/api/bootstrap?keys=aaiiSentiment'), { signal: AbortSignal.timeout(5_000) });
      if (resp.ok) {
  ...
  private renderPanel(): void {
    if (!this.data?.latest) { this.showError(t('common.noDataShort'), () => void this.fetchData()); return; }
    const d = this.data;
    ...
    const spreadDelta = previous?.spread != null ? latest.spread - previous.spread : null;
    ...
    const contrarianSignal = latest.spread <= -20
      ? `<div style="display:flex;align-items:center;gap:6px;padding:6px 8px;margin:8px 0;border-radius:4px;border:1px solid #2ecc71;background:rgba(46,204,113,0.08);font-size:10px;color:#2ecc71">&#9432; Contrarian buy signal active: spread at ${latest.spread.toFixed(1)}% (threshold: -20%)</div>`
      : latest.bearish >= 50
        ? `<div style="display:flex;align-items:center;gap:6px;padding:6px 8px;margin:8px 0;border-radius:4px;border:1px solid #e67e22;background:rgba(230,126,34,0.08);font-size:10px;color:#e67e22">&#9888; Extreme bearish reading: ${latest.bearish.toFixed(1)}% bearish (avg: ${historicalAvg.bearish}%)</div>`
        : '';
    const avgSection = avg8w ? `<div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,0.06)"><div style="font-size:10px;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.05em;margin-bottom:4px">8-Week Moving Average</div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:4px;text-align:center"><div><div style="font-size:14px;font-weight:600;color:#2ecc71">${avg8w.bullish}%</div><div style="font-size:9px;color:var(--text-dim)">Bull</div></div><div><div style="font-size:14px;font-weight:600;color:#95a5a6">${avg8w.neutral}%</div><div style="font-size:9px;color:var(--text-dim)">Neutral</div></div><div><div style="font-size:14px;font-weight:600;color:#e74c3c">${avg8w.bearish}%</div><div style="font-size:9px;color:var(--text-dim)">Bear</div></div></div></div>` : '';
    const extremeSection = (extremes.spreadBelow20 > 0 || extremes.bearishAbove50 > 0)
      ? `<div style="margin-top:6px;font-size:10px;color:var(--text-dim)">52w extremes: ${extremes.spreadBelow20} contrarian signals, ${extremes.bearishAbove50} extreme bear, ${extremes.bullishAbove50} extreme bull</div>` : '';
    const fallbackBadge = d.fallback ? '<span style="display:inline-block;padding:1px 5px;border-radius:3px;background:rgba(230,126,34,0.15);color:#e67e22;font-size:9px;margin-left:4px">(fallback data)</span>' : '';
    ...
    const html = `<div style="padding:12px 14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;text-align:center;padding:8px;background:rgba(255,255,255,0.03);border-radius:6px;margin-bottom:8px">
        <div><div style="font-size:22px;font-weight:700;color:#2ecc71">${latest.bullish.toFixed(1)}%</div><div style="font-size:10px;color:var(--text-dim)">Bullish</div><div style="font-size:9px;color:var(--text-dim)">avg ${historicalAvg.bullish}%</div></div>
        <div><div style="font-size:22px;font-weight:700;color:#95a5a6">${latest.neutral.toFixed(1)}%</div><div style="font-size:10px;color:var(--text-dim)">Neutral</div><div style="font-size:9px;color:var(--text-dim)">avg ${historicalAvg.neutral}%</div></div>
        <div><div style="font-size:22px;font-weight:700;color:#e74c3c">${latest.bearish.toFixed(1)}%</div><div style="font-size:10px;color:var(--text-dim)">Bearish</div><div style="font-size:9px;color:var(--text-dim)">avg ${historicalAvg.bearish}%</div></div>
      </div>
