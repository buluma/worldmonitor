...
interface CotInstrumentData {
  ...
  code: string;
  reportDate: string;
  assetManagerLong: string | number;
  assetManagerShort: string | number;
  leveragedFundsLong: string | number;
  leveragedFundsShort: string | number;
  dealerLong: string | number;
  dealerShort: string | number;
  netPct: number;
}
...
export class CotPositioningPanel extends Panel {
  ...
  constructor() {
    super({ id: 'cot-positioning', title: t('panels.cotPositioning'), showCount: false, infoTooltip: t('components.cotPositioning.infoTooltip') });
  }
  ...
  public async fetchData(): Promise<boolean> {
    ...
    try {
      ...
      if (resp.unavailable || !resp.instruments || resp.instruments.length === 0) {
        if (!this._hasData) this.showError(t('common.noDataShort'), () => void this.fetchData());
        return false;
      }
      this._hasData = true;
      ...
      if (!this._hasData) this.showError(e instanceof Error ? e.message : t('common.failedToLoad'), () => void this.fetchData());
      return false;
    }
  }
  ...
  private render(instruments: CotInstrumentData[], reportDate: string): void {
    ...
    const dateFooter = reportDate
      ...
      : '';
    this.setSafeContent(unsafeRawHtml(`<div style="padding:10px 14px">${rows}${dateFooter}</div>`, 'legacy Panel.setContent() migration'));
  }
}
