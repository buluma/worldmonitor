...
function alertLabel(level: string): string {
  if (level === 'alert') return t('mcp.diseaseOutbreaks.levels.alert');
  if (level === 'warning') return t('mcp.diseaseOutbreaks.levels.warning');
  return t('mcp.diseaseOutbreaks.levels.watch');
}
...
function relativeTime(ms: number): string {
  ...
  if (h < 1) return t('mcp.diseaseOutbreaks.time.justNow');
  if (h < 24) return t('mcp.diseaseOutbreaks.time.hoursAgo', { count: h });
  const d = Math.floor(h / 24);
  return t('mcp.diseaseOutbreaks.time.daysAgo', { count: d });
}
...
export class DiseaseOutbreaksPanel extends Panel {
  ...
  constructor() {
    super({
      ...
      title: t('mcp.diseaseOutbreaks.title'),
      showCount: false,
      infoTooltip: `${t('mcp.diseaseOutbreaks.infoTooltip')}<br><br><em>${t('mcp.diseaseOutbreaks.methodologyNote')}</em>`,
    });
  ...
  public async fetchData(): Promise<boolean> {
    ...
    try {
      ...
      if (!data.outbreaks?.length) {
        if (!this._hasData) this.showError(t('mcp.diseaseOutbreaks.errors.noData'), () => void this.fetchData());
        return false;
      }
      ...
      if (!this._hasData) this.showError(e instanceof Error ? e.message : t('mcp.diseaseOutbreaks.errors.failedToLoad'), () => void this.fetchData());
      return false;
    }
  }
  ...
  private _render(): void {
    ...
    const filterBar = `<div style="display:flex;gap:4px;margin-bottom:8px;flex-wrap:wrap;align-items:center">
      ${counts.alert > 0 ? `<button data-filter="alert" style="font-size:10px;padding:2px 8px;border-radius:10px;border:1px solid rgba(231,76,60,0.4);background:${this._filter === 'alert' ? 'rgba(231,76,60,0.2)' : 'transparent'};color:#e74c3c;cursor:pointer">${escapeHtml(t('mcp.diseaseOutbreaks.filters.alert', { count: counts.alert }))}</button>` : ''}
      ${counts.warning > 0 ? `<button data-filter="warning" style="font-size:10px;padding:2px 8px;border-radius:10px;border:1px solid rgba(230,126,34,0.4);background:${this._filter === 'warning' ? 'rgba(230,126,34,0.2)' : 'transparent'};color:#e67e22;cursor:pointer">${escapeHtml(t('mcp.diseaseOutbreaks.filters.warning', { count: counts.warning }))}</button>` : ''}
      ${counts.watch > 0 ? `<button data-filter="watch" style="font-size:10px;padding:2px 8px;border-radius:10px;border:1px solid rgba(241,196,15,0.4);background:${this._filter === 'watch' ? 'rgba(241,196,15,0.2)' : 'transparent'};color:#f1c40f;cursor:pointer">${escapeHtml(t('mcp.diseaseOutbreaks.filters.watch', { count: counts.watch }))}</button>` : ''}
    </div>`;
    ...
    const rows = filtered.map(o => {
      ...
      const sourceLink = o.sourceUrl
        ? `<a href="${escapeHtml(sanitizeUrl(o.sourceUrl))}" target="_blank" rel="noopener noreferrer" style="color:var(--accent-primary);text-decoration:none;font-size:9px">${escapeHtml(o.sourceName || t('mcp.diseaseOutbreaks.sourceFallback'))}</a>`
    ...
    this.setSafeContent(unsafeRawHtml(`
      ...
      <div style="margin-top:6px;font-size:9px;color:var(--text-dim)">${escapeHtml(t('mcp.diseaseOutbreaks.attribution'))}</div>
