...
function renderEntry(e: EarningsEntry): string {
  ...
  if (e.hasActuals && epsActFmt) {
    ...
    epsHtml = `<span style="font-size:11px;font-weight:600;color:var(--text)">${escapeHtml(t('components.earningsCalendar.epsActual', { value: epsActFmt }))}</span>
...
function renderGroup(date: string, entries: EarningsEntry[], isFirst: boolean): string {
  return `<div style="${isFirst ? '' : 'border-top:1px solid rgba(255,255,255,0.06);'}padding-top:${isFirst ? '0' : '10'}px;padding-bottom:2px">
...
export class EarningsCalendarPanel extends Panel {
  ...
  private render(earnings: EarningsEntry[]): void {
    ...
    const html = `<div style="padding:0 14px 12px;max-height:480px;overflow-y:auto">${sortedDates.map((d, i) => renderGroup(d, grouped.get(d)!, i === 0)).join('')}</div>`;
