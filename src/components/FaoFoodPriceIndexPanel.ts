...
function yPos(v: number, yMin: number, yMax: number): number {
  ...
  const scale = (v - yMin) / range;
  return MT + CH - scale * CH;
}
...
function buildLegend(): string {
  return SERIES.map(s =>
    `<span class="fao-legend-item"><span class="fao-legend-dot" style="background:${s.color}"></span>${escapeHtml(s.label)}</span>`
  ).join('');
}
    ...
    const headline = `
      <div class="fao-headline">
        <div class="fao-headline-primary">
          ...
          <span class="fao-index-label">FFPI</span>
        </div>
        <div class="fao-headline-changes">
          <span class="fao-change ${momCls}">${momSign}${data.momPct.toFixed(1)}% MoM</span>
          <span class="fao-change ${yoyCls}">${yoySign}${data.yoyPct.toFixed(1)}% YoY</span>
        </div>
        <div class="fao-as-of">as of ${escapeHtml(latest?.date ?? '')}</div>
      </div>`;

    const chart = buildChart(data.points);
    const legend = `<div class="fao-legend">${buildLegend()}</div>`;
    const base = `<div class="fao-base-note">Base period: 2014-2016 = 100. Source: FAO</div>`;

    this.setSafeContent(unsafeRawHtml(`<div class="fao-food-price-index-panel">${headline}${chart}${legend}${base}</div>`, 'RPC data escaped at each interpolation point'));
  }
}
