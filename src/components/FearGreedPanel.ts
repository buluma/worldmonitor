...
function getDivergenceWarnings(d: FearGreedData): string[] {
  ...
  if ((d.sentiment?.score ?? 50) < 15) warnings.push('Sentiment in extreme fear zone');
...
function renderGauge(score: number, label: string, delta: number | null, color: string): string {
  ...
  const zones = [
    { a1: 180, a2: 144, fill: '#c0392b' }, { a1: 144, a2: 108, fill: '#e67e22' },
    { a1: 108, a2:  72, fill: '#f1c40f' }, { a1:  72, a2:  36, fill: '#2ecc71' },
...
function mapSeedPayload(raw: Record<string, unknown>): FearGreedData | null {
  ...
  const mapCat = (c: Record<string, unknown> | undefined): CategoryData | undefined => c ? {
    score: Number(c.score ?? 50), weight: Number(c.weight ?? 0),
    contribution: Number(c.contribution ?? 0), degraded: Boolean(c.degraded),
  ...
  return {
    ...
    pctAbove200d: Number(hdr?.pctAbove200d?.value ?? 0), cnnFearGreed: Number(hdr?.cnnFearGreed?.value ?? 0),
    cnnLabel: String(hdr?.cnnFearGreed?.label ?? ''), aaiiBull: Number(hdr?.aaiBull?.value ?? 0),
    aaiiBear: Number(hdr?.aaiBear?.value ?? 0), fedRate: String(hdr?.fedRate?.value ?? ''),
    unavailable: false,
  };
}
...
export class FearGreedPanel extends Panel {
  ...
  constructor() {
    super({ id: 'fear-greed', title: t('panels.fearGreed'), showCount: false, infoTooltip: t('components.fearGreed.infoTooltip') });
  }
  ...
  private renderPanel(): void {
    if (!this.data) { this.showError(t('common.noDataShort'), () => void this.fetchData()); return; }
    const d = this.data;
    const score = d.compositeScore;
    const delta = d.previousScore > 0 ? score - d.previousScore : null;
    ...
    const catRows = CAT_NAMES.map(name => {
      ...
      return `<div style="margin:4px 0">
        <div style="display:flex;justify-content:space-between;font-size:11px;color:var(--text-dim)">
          <span>${escapeHtml(CAT_DISPLAY[name] ?? name)}${deg}</span>
    ...
    const warningsHtml = warnings.length > 0
      ? `<div style="margin-bottom:10px">${warnings.map(w => `<div style="display:flex;align-items:center;gap:6px;padding:5px 8px;margin-bottom:4px;border-radius:4px;border:1px solid #e67e22;background:rgba(230,126,34,0.08);font-size:10px;color:#e67e22">&#9888; ${escapeHtml(w)}</div>`).join('')}</div>`
      : '';
    ...
    const html = `
      <div style="padding:12px 14px">
        <div style="text-align:center;margin-bottom:12px">
          ...
          ${renderGauge(score, escapeHtml(d.compositeLabel), delta, color)}
        ...
        ${warningsHtml}
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:2px;background:rgba(255,255,255,0.04);border-radius:8px;padding:4px;margin-bottom:12px">${hdr}</div>
