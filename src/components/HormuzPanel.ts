...
function barChart(series: HormuzSeries[], color: string, unit: string, width = 280, height = 52): string {
  if (!series.length) return `<div style="height:${height}px;display:flex;align-items:center;color:var(--text-dim);font-size:10px">${escapeHtml(t('mcp.hormuzTracker.noData'))}</div>`;
...
function renderChart(chart: HormuzChart, idx: number): string {
  ...
  const lastVal = last ? Number(last.value).toFixed(0) : t('mcp.hormuzTracker.notAvailable');
  const lastDate = last ? last.date.slice(5) : '';
  const unit = chart.label.includes('crude_oil') ? t('mcp.hormuzTracker.units.ktPerDay') : t('mcp.hormuzTracker.units.generic');
...
export class HormuzPanel extends Panel {
  ...
  constructor() {
    super({ id: 'hormuz-tracker', title: t('mcp.hormuzTracker.title'), showCount: false, infoTooltip: t('mcp.hormuzTracker.infoTooltip') });
  }
  ...
  public async fetchData(): Promise<boolean> {
    ...
    try {
      ...
      if (!data) {
        this.showError(t('mcp.hormuzTracker.errors.unavailable'), () => void this.fetchData());
        return false;
      }
      this.data = data;
      this.renderPanel();
      ...
      this.showError(e instanceof Error ? e.message : t('mcp.hormuzTracker.errors.failedToLoad'), () => void this.fetchData());
      return false;
    }
  }
  ...
  private renderPanel(): void {
    ...
    const charts = d.charts.length
      ...
      : `<div style="color:var(--text-dim);font-size:11px;padding:8px 0">${escapeHtml(t('mcp.hormuzTracker.chartUnavailable'))}</div>`;
    ...
    const html = `
      <div style="padding:12px 14px;position:relative">
        ...
        <div style="margin-top:4px;font-size:9px;color:var(--text-dim)">
          ${escapeHtml(t('mcp.hormuzTracker.sourcePrefix'))} <a href="${escapeHtml(d.attribution.url)}" target="_blank" rel="noopener" style="color:var(--text-dim);text-decoration:underline">${escapeHtml(d.attribution.source)}</a>
        </div>
      </div>`;
