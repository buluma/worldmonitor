...
interface RawSeedPayload {
  ...
  currentPctAbove200d?: number | null;
  updatedAt?: string;
  history?: Array<{ date: string; pctAbove20d?: number | null; pctAbove50d?: number | null; pctAbove200d?: number | null }>;
  unavailable?: boolean;
}
...
const SVG_W = 480, SVG_H = 160, ML = 32, MR = 12, MT = 10, MB = 22;
...
function buildChart(points: BreadthSnapshot[]): string {
  ...
  const xAxis = points.map((p, i) => {
    ...
    return `<text x="${x.toFixed(1)}" y="${SVG_H - MB + 13}" text-anchor="middle" fill="rgba(255,255,255,0.4)" font-size="7">${escapeHtml(p.date.slice(5))}</text>`;
  }).join('');
  const areas = SERIES.map(s => splitSeriesByNulls(points, s.key as NumericSeriesKey).map(run => {
    const d = runToAreaPath(run);
    return d ? `<path d="${d}" fill="${s.color}" opacity="${s.fillOpacity}"/>` : '';
  }).join('')).join('');
  const lines = SERIES.map(s => splitSeriesByNulls(points, s.key as NumericSeriesKey).map(run => {
    if (run.length < 2) return '';
    const coords = run.map((p) => `${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ');
    return `<polyline points="${coords}" fill="none" stroke="${s.color}" stroke-width="1.5" opacity="0.9"/>`;
  }).join('')).join('');
  const midLine = yPos(50);
  return `<svg viewBox="0 0 ${SVG_W} ${SVG_H}" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;display:block">${yAxis}<line x1="${ML}" y1="${midLine.toFixed(1)}" x2="${SVG_W - MR}" y2="${midLine.toFixed(1)}" stroke="rgba(255,255,255,0.12)" stroke-width="1" stroke-dasharray="4 3"/>${xAxis}${areas}${lines}</svg>`;
}
...
export class MarketBreadthPanel extends Panel {
  ...
  constructor() {
    super({ id: 'market-breadth', title: t('panels.marketBreadth'), showCount: false, infoTooltip: t('components.marketBreadth.infoTooltip') });
  }
  ...
  private renderPanel(): void {
    if (!this.data?.history?.length) { this.showError(t('common.noDataShort'), () => void this.fetchData()); return; }
    const d = this.data;
    const chart = buildChart(d.history);
    const currentMap: Record<NumericSeriesKey, number | null> = {
      pctAbove20d: d.currentPctAbove20d, pctAbove50d: d.currentPctAbove50d, pctAbove200d: d.currentPctAbove200d,
    };
