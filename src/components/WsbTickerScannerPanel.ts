...
export class WsbTickerScannerPanel extends Panel {
  ...
  constructor() {
    super({ id: 'wsb-ticker-scanner', title: t('panels.wsbTickerScanner'), infoTooltip: t('components.wsbTickerScanner.infoTooltip'), showCount: true });
  ...
  public async fetchData(): Promise<boolean> {
    ...
    try {
      const resp = await fetch(toApiUrl('/api/bootstrap?keys=wsbTickers'), { signal: AbortSignal.timeout(5_000) });
      if (resp.ok) {
