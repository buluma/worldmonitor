...
export class YieldCurvePanel extends Panel {
  ...
  private _render(): void {
    ...
    if (this._tab === 'rates') {
      this.setSafeContent(unsafeRawHtml(`<div style="padding:10px 14px 6px">${tabBar}${renderRatesTab(this._rateRows)}</div>`, 'RPC data escaped at each interpolation point'));
      return;
    }
    ...
    this.setSafeContent(unsafeRawHtml(`
      ...
      </div>`, 'RPC data escaped at each interpolation point via escapeHtml'));
  }
}
