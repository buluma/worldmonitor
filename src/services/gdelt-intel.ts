...
// Optimized: Disabled persistCache to reduce Upstash commands, increased TTL to 30min
const gdeltBreaker = createCircuitBreaker<SearchGdeltDocumentsResponse>({ name: 'GDELT Intelligence', cacheTtlMs: 30 * 60 * 1000, persistCache: false });
const positiveGdeltBreaker = createCircuitBreaker<SearchGdeltDocumentsResponse>({ name: 'GDELT Positive', cacheTtlMs: 30 * 60 * 1000, persistCache: false });
