...
describe('Bootstrap endpoint (api/bootstrap.js)', () => {
  ...
  it('supports self-hosted local-file cache reads', () => {
    assert.ok(src.includes("WM_CACHE_BACKEND"), 'Missing cache backend selection');
    assert.ok(src.includes("local-file"), 'Missing local-file bootstrap backend');
    assert.ok(src.includes("WM_LOCAL_CACHE_FILE"), 'Missing local cache file support');
  });
