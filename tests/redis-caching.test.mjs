...
describe('local-file cache backend', { concurrency: 1 }, () => {
  it('reads and writes cached JSON without Upstash', async () => {
    ...
    try {
      const redis = await importRedisFresh();
      await redis.setCachedJson('local:test:key', { ok: true, value: 7 }, 60);

      const reread = await importRedisFresh();
      const value = await reread.getCachedJson('local:test:key');
      assert.deepEqual(value, { ok: true, value: 7 });
    } finally {
      restoreEnv();
      rmSync(tempDir, { recursive: true, force: true });
    }
  ...
  it('supports pipeline-backed sorted set operations for premium history storage', async () => {
    const tempDir = mkdtempSync(join(tmpdir(), 'wm-local-cache-'));
    const restoreEnv = withEnv({
      WM_CACHE_BACKEND: 'local-file',
      WM_LOCAL_CACHE_FILE: join(tempDir, 'cache.json'),
    ...
    try {
      const redis = await importRedisFresh();
      await redis.runRedisPipeline([
        ['SET', 'premium:item:1', JSON.stringify({ id: 'a' }), 'EX', 60],
        ['SET', 'premium:item:2', JSON.stringify({ id: 'b' }), 'EX', 60],
        ['ZADD', 'premium:index', 100, 'premium:item:1'],
        ['ZADD', 'premium:index', 200, 'premium:item:2'],
        ['EXPIRE', 'premium:index', 60],
      ], true);

      const reread = await importRedisFresh();
      const ranked = await reread.runRedisPipeline([
        ['ZREVRANGE', 'premium:index', 0, 1],
      ], true);
      assert.deepEqual(ranked[0]?.result, ['premium:item:2', 'premium:item:1']);

      const batch = await reread.getCachedJsonBatch(['premium:item:1', 'premium:item:2']);
      assert.deepEqual(batch.get('premium:item:1'), { id: 'a' });
      assert.deepEqual(batch.get('premium:item:2'), { id: 'b' });
    } finally {
      restoreEnv();
      rmSync(tempDir, { recursive: true, force: true });
    }
