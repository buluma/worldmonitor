...
// Adapted from koala73/main's version of this test: this fork's
// CountryDeepDivePanel imports ResilienceWidget via a relative path
// ('./ResilienceWidget') rather than the '@/components/...' alias upstream
// uses, and the catch branch is an inline arrow rather than a named
// renderFallback function — otherwise the same lazy-load guard.
test('ResilienceWidget stays out of the components barrel and loads through CountryDeepDivePanel dynamically', async () => {
  ...
  assert.match(deepDiveSource, /import\(['"]\.\/ResilienceWidget['"]\)/);
  assert.match(deepDiveSource, /import\(['"]\.\/ResilienceWidget['"]\)[\s\S]*?\.then\(\(\{\s*ResilienceWidget\s*\}\)\s*=>[\s\S]*?\)\s*\.catch\(/);
});
